  ...
  if (rpc.path == 'list-available-apps' && rpc.request.method == 'GET') {
    ...
    const bundledApps = Array.from(StaticCatalogs.entries()).map(([id, catalog]) => {
      ...
      return {
        ...
        : null),
        installedProfiles: ['profile:guest'],
      };
    });
